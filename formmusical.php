...
elseif (isset($_GET["acao"]) && $_GET["acao"] == "insere"){
    $g = new Musical();
...
elseif (isset($_GET["acao"]) && $_GET["acao"] == "altera"){
    $g = new Musical();
    $g-> setIdMusical($_POST["idMusical"]);
    $g->setArtista($_POST["artista"]);
    $g->setLocal($_POST["local"]);
    $g->setDia($_POST["dia"]);
    $musical = $g->alterar();
...
<html>
    ...
    <body>
        ...
        if(isset($_GET["acao"])) {echo $_GET["acao"];} ?>">
            <input type="hidden" name="idmusical" value="<?php
        if(isset($musical[0]["idmusical"])) {echo $musical[0]["idmusical"];} ?>">
        ...
        if(isset($musical[0]["local"]) && $musical[0]["local"] == "Teatro do Bourbon Country" ) { echo "selected"; } ?>>Teatro do Bourbon Country</option>
            ...
            Data: <input type = "text" name="dia" value="<?php
